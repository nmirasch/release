...
Related: [GITOPS-9648](https://redhat.atlassian.net/browse/GITOPS-9648)
...
The same `.npmrc` is mirrored under:

- `prefetch/yarn/` — Yarn v1 bootstrap prefetch (`{"type": "npm", "path": "prefetch/yarn"}`)
- `prefetch/pnpm/` — pnpm bootstrap prefetch (`{"type": "npm", "path": "prefetch/pnpm"}`)
...
3. **`.netrc` Secret** — if Nexus requires authentication (e.g. Sonatype User Token), bind a `netrc` workspace on each npm/yarn/pnpm component PipelineRun. **Never commit credentials to this repository.**
...
## Components using npm/yarn/pnpm prefetch

See `prefetch/yarn/README.md` and `prefetch/pnpm/README.md` for bootstrap workarounds and Tekton `prefetch-input` examples.
